...
@[simp]
theorem ι_square_scalar (m : M) : ι R m * ι R m = algebra_map R _ (Q m) :=
begin
  dsimp [ι],
...
theorem ι_comp_lift (f : M →ₗ[R] A) (cond : ∀ m, f m * f m = algebra_map _ _ (Q m)) :
  (lift R M Q f cond).to_linear_map.comp (ι R Q) = f :=
by { ext, simp [lift, ι] }
...
(g : clifford_algebra R M Q →ₐ[R] A) :
  g.to_linear_map.comp (ι R Q) = f ↔ g = lift R M Q f cond :=
begin
...
by rw [←alg_hom.map_mul, ι_square_scalar, alg_hom.map_zero]

@[simp]
theorem lift_comp_ι (g : clifford_algebra R M Q →ₐ[R] A) :
  lift R M Q (g.to_linear_map.comp (ι R Q)) (comp_ι_square_scalar _) = g :=
